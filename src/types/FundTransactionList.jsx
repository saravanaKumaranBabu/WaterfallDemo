...
import { useFetchFundTransactions } from "../Hooks/FundHooks"
...
const FundTransactionList = (props) => {
  ...
  const { data } = useFetchFundTransactions();
                  ...
                  data.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCell onClick={() => { navigateToAction(row) }}
                        style={{ height: '42px', textAlign: 'center', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }}>{row.batchId || ''}</TableCell>
                      <TableCell onClick={() => { navigateToAction(row) }}
                        style={{ fontWeight: '500', fontSize: '14px', lineHeight: '24px', textAlign: 'center' }}>{format(new Date(row.gldate), "MM/dd/yyyy") || ''}</TableCell>
                      <TableCell onClick={() => { navigateToAction(row) }}
                        style={{ fontWeight: '500', fontSize: '14px', lineHeight: '24px', textAlign: 'center' }}>{format(new Date(row.effectiveDate), "MM/dd/yyyy") || ''}</TableCell>
                      <TableCell onClick={() => { navigateToAction(row) }}
                        style={{ textAlign: 'left', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }}>{row.event}</TableCell>
                      {/* {
                    row.event == 'Contribution' ?
                      <TableCell onClick={ ()=> {navigateToAction(row)}} 
                        style={{ color: '#FF5300' }}>{row.event}</TableCell>
                      :
                      row.event == 'Unrealized' ?
                        <TableCell onClick={ ()=> {navigateToAction(row)}} 
                          style={{ color: '#219653' }}>{row.event}</TableCell>
                        :
                        row.event == 'Distribution' ?
                          <TableCell onClick={ ()=> {navigateToAction(row)}} 
                            style={{ color: '#115AC8' }}>{row.event}</TableCell>
                          :
                          row.event == 'Capital Call' ?
                            <TableCell onClick={ ()=> {navigateToAction(row)}} 
                              style={{ color: '#FE2762' }}>{row.event}</TableCell>
                            :
                            <TableCell onClick={ ()=> {navigateToAction(row)}}>
                              {row.event}</TableCell>
                  } */}
                      <TableCell onClick={() => { navigateToAction(row) }} style={{ textAlign: 'left', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }}>
                        {row.deal || ''}</TableCell>
                      <TableCell onClick={() => { navigateToAction(row) }}
                        style={{
                          textAlign: 'right', color: (row.amount >= 0) ? '#21234A' : '#EA3323', fontWeight: '500', fontSize: '14px',
                      ...
                      <TableCell style={{ textAlign: 'center', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }} onClick={() => { navigateToAction(row) }}>
                        {row.investranStatus || ''}</TableCell>
                      <TableCell style={{ textAlign: 'center', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }} onClick={() => { navigateToAction(row) }}>
                        {row.status || ''}</TableCell>
                      <TableCell style={{ textAlign: 'center', fontWeight: '500', fontSize: '14px', lineHeight: '24px' }} onClick={() => { navigateToAction(row) }}>
                        {format(new Date(row.effectiveDate), "MM/dd/yyyy") || ''}</TableCell>
                      <TableCell className="actions-menu-main" tabIndex={-1} >
                        <div className="open-actions-icon justify-content-center" >
                          {row.event === "Distribution" || row.event === "Accrued Carry" ?
                            <>
                              <OverlayTrigger
                                delay={{ hide: 450, show: 300 }}
                                overlay={(props) => (
                                ...
                                <button id={'viewInputWfeBtn-'+index} className="options-button me-2">
                                  <Link to={"/actionpage/viewInput"} state={{ path: 'dashboard', event: row.event, active: 'viewInput' }}
                              ...
                              <OverlayTrigger
                                ...
                                <button id={'viewOutputWfeBtn-'+index} className="options-button me-2">
                                  <Link to={"/actionpage/viewOutput"} state={{ path: 'dashboard', event: row.event, active: 'viewOutput' }}
                                    style={{ fontWeight: '500', color: '#344050' }}>
